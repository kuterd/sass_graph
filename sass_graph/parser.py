...
class Operand:
    ...
    def from_json_obj(cls, obj):
        ...
        elif otype == "LabelOperand":
            return LabelOperand.from_json_obj(obj)
...
class RegOperand(Operand):
    ...
    def __repr__(self):
        no_seperator = ["B", "R", "P", "UR"]

        base = (
            self.reg_type + ("" if self.reg_type in no_seperator else "_") + self.ident
        )
...
class LabelOperand(Operand):
    ...
    @classmethod
    def from_json_obj(cls, obj):
        return cls(label_name=obj["label_name"])
...
class _InstructionParser:
    ...
    def _parseLabel(self, s):
        return LabelOperand(s[2:-1])
